...
public class GuruStockAggregator {
	...
	// MAIN FUNCTION
	public static void printGuruStockData(String guru,
			LinkedList<GuruTicker> tickers) throws Exception {
		int pageNum = 0;
		int numDone = 0;
		...
		do {
			...
			for (GuruTicker t : tickers) {
				...
				for (Object obj : nodes) {
					...
					t.entryList.addStockEntry(date, entryType, fAvgPrice,
							fMinPrice, numShares);
				}
			}
	...
	private static String getNextTxPage(String relativeURL) throws Exception {
		return Utils.getWebPageContents("http://www.gurufocus.com"
				+ relativeURL);
	}
...
class StockEntryDoubleLinkedList {
	...
	public void adjustTxShares() // adjusts the shares from total number of
	...
	public void printOldestToLatest() {
		...
		do {
			System.out.printf("%s\t %s\t %.2f\t %.2f\t %,d\n", se.date,
					se.entryType, se.avgPrice, se.minPrice, se.numShares);
...
class StockEntry {
	...
	public StockEntry(String aDate, String aEntryType, float aAvgPrice,
			float aMinPrice, int aNumShares) {
		date = aDate;
