...
public final class Money implements Comparable<Money>, Serializable {
	...
	public static final class MismatchedCurrencyException extends RuntimeException {
		...
		private static final long serialVersionUID = 8595118819020031951L;
	...
	public Money(String aAmount) {
		this(new BigDecimal(aAmount.trim()));
	}
	...
	public long inCents() {
		return (long) (getAmount().floatValue() * 100);
	}

	public static Money fromCents(Long cents) {
		return new Money(new BigDecimal(cents * 0.01));
	}

	/**
	 * Returns { @link #getCurrency()}.getSymbol() + @link #getAmount()}.getPlainString().
	 * 
	 * <P>
...
//		NumberFormat.getCurrencyInstance().format(fAmount);
		return NumberFormat.getCurrencyInstance().format(fAmount); //fAmount.toPlainString();
	}
	...
	private void round() {
		fAmount = fAmount.setScale(2, fRounding);
	}
	...
	private void checkCurrenciesMatch(Money aThat) {
		if (!this.fCurrency.equals(aThat.getCurrency())) {
			throw new MismatchedCurrencyException(aThat.getCurrency() + " doesn't match the expected currency : "
					+ fCurrency);
		}
	}
